// This bundler doesn't resolve tmalign-wasm's own bare `.wasm` import as a
// URL, so we reimplement its wrapper here using Vite's explicit `?url`
// suffix, which reliably works for any asset type. `parse`/`parseMatrix`
// are copied verbatim from tmalign-wasm/tmalign.js (pure JS, no wasm).
import createTmalign from 'tmalign-wasm/tmalign-wasm.js';
import tmalignWasmUrl from 'tmalign-wasm/tmalign-wasm.wasm?url';

export function tmalign(pdb1, pdb2, alignment = null) {
    return new Promise((resolve, reject) => {
        let buffer = "";
        createTmalign({
            locateFile: () => tmalignWasmUrl,
